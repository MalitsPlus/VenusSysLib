...
export function applyContinuousEffects(
  ...
  this.current.cardStatuses.forEach(cardStat => {
    const recoveryValue = cardStat.getEffectValue(SkillEfficacyType.StaminaContinuousRecovery)
    if (recoveryValue) {
    ...
    const consumeValue = cardStat.getEffectValue(SkillEfficacyType.StaminaContinuousConsumption)
    if (consumeValue) {
